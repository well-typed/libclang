...
main :: IO ()
main = do
    bs <- BS.readFile "libclang-bootstrap/imports.h"
    ...
    do
        ...
        putStr contents
        writeFile "libclang-bindings/src/Clang/LowLevel/FFI.hs" contents

    do
        let contents = wrappers decls
        putStr contents
        writeFile "libclang-bindings/cbits/clang_wrappers_ffi.h" contents
...
ifdefP :: Parser Decl
ifdefP = P.try $ do
    _ <- lexeme (P.string "#ifdef")
    name <- cident
    return (Ifdef name)

endifP :: Parser Decl
endifP = Endif <$ lexeme (P.string "#endif")
